 ...
 */
export async function ensurePriceId(): Promise<string> {
...
type SubscriptionState = 'trialing' | 'active' | 'past_due' | 'canceled';
 ...
 */
export async function findRestaurantByCustomerId(customerId: string): Promise<{ id: string; owner_phone: string | null } | null> {
  const result = await query<{ id: string; owner_phone: string | null }>(
    `SELECT id, owner_phone FROM restaurants WHERE stripe_customer_id = $1 LIMIT 1`,
    [customerId]
  );
 ...
 */
export async function findRestaurantBySubscriptionId(subscriptionId: string): Promise<{ id: string; owner_phone: string | null; subscription_state: string | null } | null> {
  const result = await query<{ id: string; owner_phone: string | null; subscription_state: string | null }>(
