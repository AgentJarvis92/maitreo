 ...
 */
function formatApprovalSms(review, draftReply, restaurant) {
 ...
 */
async function mockPostReply(review, replyText) {
 ...
 */
export async function processReview(review, restaurant) {
    try {
        // 1. Classify sentiment
        const sentimentResult = classifySentiment(review.rating, review.text || '');
        ...
        const replyOutput = await generateReply(review, restaurant);
        ...
        return { reviewId: review.id, sentiment: 'unknown', action: 'error', error: error.message };
    }
}
/**
 * Process all new reviews for a restaurant.
 */
export async function processNewReviews(reviews, restaurant) {
    const results = [];
