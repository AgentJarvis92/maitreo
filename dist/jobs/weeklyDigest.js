...
async function seedMockReviews(restaurantId) {
    ...
    for (let i = 0; i < MOCK_REVIEWS.length; i++) {
        ...
        await query(`INSERT INTO reviews (restaurant_id, platform, review_id, author, rating, text, review_date)
       VALUES ($1, $2, $3, $4, $5, $6, NOW() - INTERVAL '${daysAgo} days')
       ON CONFLICT (platform, review_id) DO NOTHING`, [restaurantId, r.platform, `mock-digest-${i}`, r.author, r.rating, r.text]);
    }
    console.log(`✅ Seeded ${MOCK_REVIEWS.length} mock reviews`);
}
// ─── 1. Aggregation Queries ──────────────────────────────────────────
async function aggregateReviews(restaurantId, periodStart, periodEnd) {
    // Fetch reviews in window
    const reviewsResult = await query(`SELECT id, author, rating, text, review_date, platform
     FROM reviews
     WHERE restaurant_id = $1
       AND review_date >= $2
       AND review_date < $3
     ORDER BY review_date DESC`, [restaurantId, periodStart.toISOString(), periodEnd.toISOString()]);
    ...
    const statsResult = await query(`SELECT
     ...
     FROM (
       ...
       GROUP BY rating
     ) sub`, [restaurantId, periodStart.toISOString(), periodEnd.toISOString()]);
    // Daily counts
    const dailyResult = await query(`SELECT TO_CHAR(review_date AT TIME ZONE 'UTC', 'YYYY-MM-DD') AS day, COUNT(*)::int AS cnt
     FROM reviews
     WHERE restaurant_id = $1 AND review_date >= $2 AND review_date < $3
     GROUP BY day ORDER BY day`, [restaurantId, periodStart.toISOString(), periodEnd.toISOString()]);
    const dailyCounts = {};
    for (const row of dailyResult.rows) {
        dailyCounts[row.day] = row.cnt;
    }
    // Response rate: reviews that have a reply_draft with status 'approved' or 'sent'
    const responseResult = await query(`SELECT COUNT(DISTINCT rd.review_id)::int AS responded
     FROM reply_drafts rd
     JOIN reviews r ON r.id = rd.review_id
     WHERE r.restaurant_id = $1 AND r.review_date >= $2 AND r.review_date < $3
       AND rd.status IN ('approved', 'sent')`, [restaurantId, periodStart.toISOString(), periodEnd.toISOString()]);
...
// ─── 3. Email Rendering ─────────────────────────────────────────────
async function renderDigestEmail(restaurant, stats, themes, periodStart, periodEnd) {
...
// ─── 4. Send Email ───────────────────────────────────────────────────
async function sendDigestEmail(to, restaurantName, html) {
    try {
...
// ─── 5. Send SMS Summary ────────────────────────────────────────────
async function sendDigestSms(phone, stats) {
...
// ─── 6. Store Digest ────────────────────────────────────────────────
async function storeDigest(restaurantId, periodStart, periodEnd, stats, themes, emailSent, smsSent) {
    const result = await query(`INSERT INTO digests (
...
export async function generateDigest(restaurantId) {
    console.log('\n📊 ═══════════════════════════════════════');
    ...
    const restaurantsResult = await query(restaurantQuery, params);
    ...
    for (const restaurant of restaurantsResult.rows) {
        ...
        try {
            ...
            await query(`INSERT INTO email_logs (type, to_email, subject, status, sent_at, metadata)
