...
export class CompetitorTracker {
    ...
    GROWTH_THRESHOLD = 2.0; // 2x normal velocity = anomaly
    RATING_CHANGE_THRESHOLD = 0.3; // 0.3 star change = significant
    /**
     * Track all competitors for a restaurant
     */
    async trackCompetitors(restaurant) {
     ...
     */
    async getCompetitorMetrics(competitor, restaurantId) {
        try {
            let currentRating = 0;
     ...
     */
    calculateReviewVelocity(history, currentCount) {
        if (history.length < 2)
            return 0;
     ...
     */
    calculateRatingTrend(history, currentRating) {
        if (history.length === 0)
            return 'stable';
        const oldRating = history[0].rating || 0;
        const change = currentRating - oldRating;
        if (Math.abs(change) < this.RATING_CHANGE_THRESHOLD)
            return 'stable';
     ...
     */
    detectGrowthAnomaly(history, currentCount) {
        if (history.length < 3)
     ...
     */
    async saveSnapshot(restaurantId, competitor, rating, reviewCount) {
        try {
     ...
     */
    async findNearbyCompetitors(restaurantName, location, cuisine) {
        try {
            ...
            const competitors = results.businesses
...
//# sourceMappingURL=competitorTracker.js.map